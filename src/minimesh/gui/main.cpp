...
int main(int argc, char *argv[]) {
    ...
    GLUI_RadioGroup *radio_group_mouse_func = globalvars::glui->add_radiogroup_to_panel(panel_mouse_func,
                                                                                        &globalvars::viewer.get_mouse_function());
    ...
    //
    GLUI_Button *button_subdivide = globalvars::glui->add_button("Subdivide Loop", -1,
                                                                 freeglutcallback::subdivide_pressed);
    button_subdivide->set_w(200);

    GLUI_Button *button_fixed_param = globalvars::glui->add_button("Harmonic Parameterization", -1,
                                                                   freeglutcallback::fixed_param_pressed);
    button_fixed_param->set_w(200);

    GLUI_Button *button_free_param = globalvars::glui->add_button("LSCM Parameterization", -1,
                                                                  freeglutcallback::free_param_pressed);
    ...
    GLUI_Spinner *spinner_simplify = globalvars::glui->add_spinner("# of entities to simplify", GLUI_SPINNER_INT,
                                                                   &globalvars::num_entities_to_simplify);
    ...
    GLUI_Spinner *spinner_target_simplify = globalvars::glui->add_spinner("Simplify to # of vertices", GLUI_SPINNER_INT,
                                                                          &globalvars::target_num_of_vertices);
    spinner_target_simplify->set_alignment(GLUI_ALIGN_CENTER);
    spinner_target_simplify->set_w(300);

    GLUI_Button *button_target_simplify = globalvars::glui->add_button("Simplify To Target", -1,
                                                                       freeglutcallback::simplify_target_pressed);
