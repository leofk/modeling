...
void
Mesh_simplify::color_queue(Mesh_buffer &buffer, Mesh_connectivity::Defragmentation_maps &defrag, int num_e) {
...
// simplify to target
//
void Mesh_simplify::simplify_to_target(int num_of_target_entities) {

    int count = 0;
    while (num_of_target_entities != mesh().n_active_vertices()) {
        while (!errorQueue.empty()) {
...
void Mesh_simplify::construct_edge_from_history(HistoryEntry &entry) {
    ...
    new_faces.push_back(F1.index());
    new_faces.push_back(F2.index());
